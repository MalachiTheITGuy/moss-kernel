...
use memory::{
    PAGE_OFFSET,
    uaccess::{
        X86_64CopyFromUser, X86_64CopyStrnFromUser,
        X86_64CopyToUser, try_copy_from_user,
    },
};
...
impl VirtualMemory for X86_64 {
    type PageTableRoot =
        memory::address_space::X86_64PageTableRoot;
    type ProcessAddressSpace =
        memory::address_space::X86_64ProcessAddressSpace;
    type KernelAddressSpace =
        memory::mmu::X86_64KernelAddressSpace;

    fn kern_address_space(
    ) -> &'static SpinLock<Self::KernelAddressSpace> {
        memory::mmu::kern_address_space()
    }
}
...
impl Arch for X86_64 {
    ...
    fn do_signal(
        ...
        Output = Result<<Self as Arch>::UserContext>,
    > {
    ...
    fn get_cmdline() -> Option<String> {
        ...
        if cmd.is_empty() { None } else { Some(String::from(cmd)) }
    }
