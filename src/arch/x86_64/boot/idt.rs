...
pub fn ist_df_top() -> u64 {
    unsafe { IST_DF_STACK.as_ptr().add(IST_DF_STACK.len()) as u64 }
}

/// Return the top of the NMI IST stack.
pub fn ist_nmi_top() -> u64 {
    unsafe { IST_NMI_STACK.as_ptr().add(IST_NMI_STACK.len()) as u64 }
}

/// Return the top of the machine-check IST stack.
pub fn ist_mc_top() -> u64 {
    unsafe { IST_MC_STACK.as_ptr().add(IST_MC_STACK.len()) as u64 }
}
